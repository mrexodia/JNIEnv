...
static std::vector<const char*> JNIEnvFunctions = {
    "void *reserved0",
    "void *reserved1",
    "void *reserved2",
    "void *reserved3",
...
static duint JNIEnvStart = 0;
static std::unordered_map<duint, const char*> functionMap;

template<size_t Size>
static bool getLabelJNIEnv(duint addr, char(&label)[Size])
{
    auto found = functionMap.find(addr);
    if(found == functionMap.end())
        return false;
    strncpy_s(label, found->second, _TRUNCATE);
    return true;
}
...
PLUG_EXPORT void CBADDRINFO(CBTYPE cbType, PLUG_CB_ADDRINFO* info)
{
    if(!info->retval && (info->addrinfo->flags & flaglabel))
        info->retval = getLabelJNIEnv(info->addr, info->addrinfo->label);
}
...
static bool cbCommand(int argc, char* argv[])
{
    ...
    if(!DbgMemIsValidReadPtr(start))
        return false;
    functionMap.clear();
    for(size_t i = 0; i < JNIEnvFunctions.size(); i++)
    {
        duint func = 0;
        if(DbgMemRead(start + i * sizeof(duint), &func, sizeof(func)) && DbgMemIsValidReadPtr(func))
            functionMap[func] = JNIEnvFunctions[i];
    }
    GuiUpdateAllViews();
    return true;
}
